...
class MRCSRec(SequentialRecommender):
    r"""
    ...
    def __init__(self, config, dataset):
        super(MRCSRec, self).__init__(config, dataset)
    ...
    def meta_contrast(self, sequence,sequence_output_0, sequence_output_1,meta_regular,target_id):
        ...
        logits, label = self.info_nce(s_2_0, sequence_output_0, self.tau, batch_size, self.sim,target_id)
        cl_loss += nn.CrossEntropyLoss()(logits, label)
        return cl_loss
    ...
    def calculate_loss(self, interaction,regular=None,mode=None):
        ...
        sem_nce_logits, sem_nce_labels = self.info_nce(aug_seq_output, sem_aug_seq_output, temp=self.tau,
                                                       batch_size=item_seq_len.shape[0], sim=self.sim,target_id=target_id)


        loss += self.lmd_sem * self.aug_nce_fct(sem_nce_logits, sem_nce_labels)

        if mode == "step1":
            loss += self.beta * self.meta_contrast(seq_output, aug_seq_output, sem_aug_seq_output, regular,target_id)
            return loss
        elif mode == "step2":
            loss += self.meta_contrast(seq_output, aug_seq_output, sem_aug_seq_output, regular,target_id)
            return loss
        elif mode == "step3":
            loss += self.beta * self.meta_contrast(seq_output, aug_seq_output, sem_aug_seq_output, regular,target_id)
            return loss
    ...
    def info_nce(self, z_i, z_j, temp, batch_size, sim='dot',target_id=None):
        """
        ...
        if self.de_noise==True:
            mask = self.mask_correlated_samples_(target_id)
    ...
    def full_sort_predict(self, interaction,noise_ratio=None):
        item_seq = interaction[self.ITEM_SEQ]
        item_seq_len = interaction[self.ITEM_SEQ_LEN]
        # add_noise
        # item_seq, item_seq_len = add_noise_ratio(noise_ratio, item_seq, item_seq_len, self.n_items)
        seq_output = self.forward(item_seq, item_seq_len)
