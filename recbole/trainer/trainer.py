...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.use_regular=config['use_regular']
        self.regular = Extractor([64, 32, 16, 64], "gelu", None).to(self.device)
        # meta learning
        self.optimizer_1 = self._build_optimizer([{"params": self.regular.parameters()}])
        # joint learning
        self.optimizer_2 = self._build_optimizer([{"params": self.model.parameters()},{"params": self.regular.parameters()}])
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in iter_data:
            ...
            if self.joint == 1:
                ...
                losses = loss_func(interaction, self.regular,"step1")
                ...
                for param in self.regular.parameters():
                ...
                losses = loss_func(interaction, self.regular, "step2")
                if isinstance(losses, tuple):
                    loss = sum(losses)
                    loss_tuple = tuple(per_loss.item() for per_loss in losses)
                    total_loss = loss_tuple if total_loss is None else tuple(map(sum, zip(total_loss, loss_tuple)))
                ...
                for param in self.regular.parameters():
                    param.requires_grad = False
                losses = loss_func(interaction, self.regular, "step3")
                if isinstance(losses, tuple):
                    loss = sum(losses)
                    loss_tuple = tuple(per_loss.item() for per_loss in losses)
                    total_loss = loss_tuple if total_loss is None else tuple(map(sum, zip(total_loss, loss_tuple)))
    ...
    def _full_sort_batch_eval(self, batched_data,noise_ratio=None):
        interaction, history_index, swap_row, swap_col_after, swap_col_before = batched_data
        try:
            # Note: interaction without item ids
            scores = self.model.full_sort_predict(interaction.to(self.device),noise_ratio)
    ...
    @torch.no_grad()
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False,noise_ratio=None):
        r"""Evaluate the model based on the eval data.
        ...
        for batch_idx, batched_data in iter_data:
            if eval_data.dl_type == DataLoaderType.FULL:
                interaction, scores = self._full_sort_batch_eval(batched_data,noise_ratio)
            else:
