...
class SequentialDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, dl_format=InputType.POINTWISE, shuffle=False, phase='train'):
        ...
        self.phase = phase
        if config['SSL_AUG'] == 'MRCSRec' and self.phase == 'train':
    ...
    def data_preprocess(self):
        ...
        # used for MRCSRec semantic positive sampling
        if self.config['model'] == 'MRCSRec':
    ...
    def _shuffle(self):
        if self.real_time:
            ...
            if self.config['SSL_AUG'] == 'MRCSRec' and self.phase == 'train':
    ...
    def _get_processed_data(self, index):
        ...
        if self.config['SSL_AUG'] == 'MRCSRec' and self.phase == 'train':
            self.mrcsrec_aug(cur_data, index)
        
        return cur_data
    
    def mrcsrec_aug(self, cur_data, index):
